...
namespace GraphSearching {
    ...
    class GraphNode<T> {
        ...
        public bool RemoveNeighbor(GraphNode<T> neighbor) {
            ...
            int neighborLocation = neighbors.IndexOf(neighbor);
            if (neighborLocation > -1) {
                neighbors.RemoveAt(neighborLocation);
                weights.RemoveAt(neighborLocation);
                return true;
            }
            return false;
        }
        ...
        public override string ToString() {
            ...
            for (int i = 0; i < neighbors.Count; i++) {
                nodeString.Append(neighbors[i].Value + $"[{weights[i]}] ");
            }
            nodeString.Append("]");
