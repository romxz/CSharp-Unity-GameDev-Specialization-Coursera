...
public class CharacterChanger : MonoBehaviour {
    ...
    // flag support for single frame change on left mouse
    bool prevFrameChangeCharacterInput = false;
    ...
    void Update() {
        ...
        if (Input.GetAxis("ChangeCharacter") > 0) {
            if (!prevFrameChangeCharacterInput) {
                // set flag so we dont change character multiple times
                prevFrameChangeCharacterInput = true;
            ...
            prevFrameChangeCharacterInput = false;
        }
    }
}
